...
public class Return_enemy : EnemyState
{
    float lostDelay = 0.25f;
    float lostTimer;

    public Return_enemy(Enemy enemy, EnemyFSM fsm) : base(enemy, fsm) { }
    ...
    public override void LogicUpdate()
    {
        ...
        if (!enemy.InDetectRange()) lostTimer += Time.deltaTime;

        if (lostTimer >= lostDelay)
        {
            fsm.ChangeState(enemy.GoHomeState);
            return;
        }
    }
}